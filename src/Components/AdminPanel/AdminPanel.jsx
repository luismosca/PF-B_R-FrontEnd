    ...
    <div className="overview">
      ...
      <div className="boxes">
        <div className="box box1">
          ...
          <span className="number">{reports.total ? reports.total : 0}</span>{' '}
        </div>
        <div className="box box2">
          <i className="uil uil-comments"></i>
          <span className="text">Total Pending Comments</span>
          <span className="number">{comments.total ? comments.total : 0}</span>{' '}
        </div>
...
const UsuariosTable = ({ users, openModal, setSelectedUser }) => {
  return (
    <div className="activity">
      ...
      {
        users === undefined ? <div><span style={{ fontSize: "1.5rem", marginTop: "5rem" }}>No currently registered/logged Users</span></div> : null
      }
    </div>
  );
};
    ...
    try {
      ...
      if (response.status === 200) {
        Swal.fire({
          position: 'center',
          icon: 'success',
          title: `Rol del usuario: ${user.name_surName} actualizado a ${selectedRole.toUpperCase()} `,
          showConfirmButton: false,
          timer: 7500
        }).then(() => {
        ...
        console.log(`Actualización exitosa para ${user.name_surName} a ${selectedRole}`);
      } else {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <button
          className={`apply-changes-button${selectedRole.toLowerCase() === user.role.toLowerCase()
            ? '-disabled'
            : ''
            }`}
