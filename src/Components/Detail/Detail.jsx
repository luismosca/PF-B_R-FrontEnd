...
const ReportDetail = () => {
  ...
  return (
    <div className={style.detailContainer}>
      ...
      {
        user && user.id ? 
        <div className={style.comentarios}><Comments ReportId={id} /></div> : 
        <div 
        style={{ margin: "0.5rem", marginLeft: "auto", marginRight: "auto", textAlign: "center", backgroundColor: "#c5f6639f", height: "120px", fontSize: "20px" }}
        >
        <div>Inicia session para comentar en el reporte</div>
        <button className={style.commentButton}>
        <Link to="/login" style={{ padding: "20px"}}>Iniciar Sesión</Link>
        </button>
        </div>
      }
      ...
      <div>
        {comentarios?.map((coment, index) => (
          ...
          />
            <h4 htmlFor="usrname" style={{ color: "#6474459f" }}>{user.name}</h4>
            <label htmlFor="date">{coment.createdAt.split("T")[0]}</label>
            <span>{coment.comment}</span>
          </div>
        ))}
      </div>
      <div style={{ margin: "0.5rem", marginLeft: "auto", marginRight: "auto", textAlign: "center",  }}>
        <h3 style={{backgroundColor: "#c5f6639f", width: "150px", color: "black", borderRadius:"50px", border: "1px groove black"}}>Comentarios:</h3>
      </div>
    </div>
  );
};
